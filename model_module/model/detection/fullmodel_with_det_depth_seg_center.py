...
class FullModel(nn.Module):
    def __init__(self, backbone, 
        ...
        if norm == 'BN':
            ...
            # self.pred_seg = nn.Sequential(ConvNeXtBlock(dim*6, 0.0),     
            #                         ConvNeXtBlock(dim*6, 0.0))
            
            self.pred_det = nn.Sequential(ConvNeXtBlock(256, 0.0),     
                                    ConvNeXtBlock(256, 0.0))
            
            # self.to_logits = nn.Sequential(
            #                     nn.Conv2d(dim*6, int(dim*6/2), 3, padding=1, bias=False),
            #                     LayerNorm(int(dim*6/2), eps=1e-6, data_format = "channels_first"),
            #                     nn.GELU(),
            #                     nn.Conv2d(int(dim*6/2), len(outputs.keys()), 1))
    ...
    def forward(self, sample):
        ...
        bev_1 = self.q_generator(F128, I, E)      # b d bh bw

        # bev_seg = self.pred_seg(bev_1)                    # b 2 bh bw
        # pred = self.to_logits(bev_seg)

        bev_det = self.pred_det(bev_1) 
