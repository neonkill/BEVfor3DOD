...
class SELFDETR_HEAD(nn.Module):
    def __init__(self, num_class, in_channels, num_block, num_decoder_layers, transformer, pc_range, bev_embedding):
        super().__init__()
        ...
        self.query_embed = nn.Embedding(900, self.embed_dims)
    ...
    def forward(self, bev_feat):
        ...
        output = self.transformer(query=query, query_pos=query_pos, key=key , value=value, key_pos=bev_embed)

        outputs_classes = []
        outputs_coords = []
        for i in range(output.shape[0]):
            outputs_class = self.cls_branches[i](output[i])
            tmp = self.reg_branches[i](output[i])
