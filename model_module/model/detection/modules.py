...
class Aggregator_LN_GELU_NoReduce(nn.Module):

    def __init__(self, in_dim, bins, target_idx):
        super(Aggregator_LN_GELU_NoReduce, self).__init__()

        self.target_idx = target_idx
        self.aggregator = ConvBlock(in_dim*len(bins), in_dim*len(bins), 1, norm='LN', act='GELU')
