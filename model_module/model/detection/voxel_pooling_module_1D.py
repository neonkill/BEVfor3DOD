...
class VoxelModule(nn.Module):
    ...
    def create_frustum(self, batch_size, gt_depth=None):
        """Generate frustum"""
        # make grid in image plane
        ogfH, ogfW = self.final_dim
        fH, fW = ogfH // self.downsample_factor, ogfW // self.downsample_factor
        
        gt_depth = gt_depth.view(batch_size, 6, 1, fH, fW)
    ...
    def get_geometry(self, sensor2ego_mat, intrin_mat, ida_mat, bda_mat, depth):
        ...
        points = self.create_frustum(batch_size, depth)
        
        ida_mat = ida_mat.view(batch_size, num_cams, 1, 1, 1, 4, 4)
        points = ida_mat.inverse().matmul(points.unsqueeze(-1))
            
        # print(points_aug[:,:,:,:,:,2].unique())
        # exit()
        # cam_to_ego
        points = torch.cat(
            (points[:, :, :, :, :, :2] * points[:, :, :, :, :, 2:3],
    ...
    def _forward_single_sweep(self,
        ...
        img_feat_with_depth = seg[:, sweep_index, ...].unsqueeze(2) # b*n , 64, 112, 64, 176
