...
def generate_info(nusc, scenes, max_cam_sweeps=0, max_lidar_sweeps=0):
    infos = list()
...
def main():
    trainval_nusc = NuScenes(version='v1.0-trainval',
                             dataroot='/usr/src/nuscenes/',
                             verbose=True)
    train_scenes = splits.train
    val_scenes = splits.val
    train_infos = generate_info(trainval_nusc, train_scenes)
    val_infos = generate_info(trainval_nusc, val_scenes)
    mmcv.dump(train_infos, '/usr/src/nuscenes/nuscenes_infos_new_train.pkl')
    mmcv.dump(val_infos, '/usr/src/nuscenes/nuscenes_infos_new_val.pkl')
